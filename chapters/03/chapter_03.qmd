---
title: "Chapter 3: Stationary Time Series Models"
author: ""
abstract: |
...
determinants. A natural starting point is a **distributed lag (DL) model**: regress
...
of freedom disappear quickly, and the estimates become noisy.
...
Initial jobless claims (`ICSA`) — the number of first-time unemployment insurance
...
Department of Labour, it gives financial markets a near-real-time read on the pace
...
through by $\gamma(0)$ gives the **Yule-Walker equations**:
...
The decomposition separates $y_t$ into two orthogonal pieces. The first is the
part that is genuinely surprising — driven by innovations that could not have been
foreseen from past data. The second is the part that is completely foreseeable from
the past, like a deterministic trend or a perfectly regular periodic component. For
most economic time series, the deterministic part is either absent or trivially
handled by demeaning, so we focus on the stochastic part. The key object is the
MA($\infty$) representation: the sequence $\{\psi_j\}$ and the innovation process
$\{\varepsilon_t\}$.

### Why This Matters

The Wold theorem is not just a mathematical curiosity — it is the foundation on
...
The **ARMA($p$,$q$)** model combines both components:
...
This matters in practice: when you fit an ARMA model in Python, the reported
...
Information criteria formalise this tradeoff. They all take the same conceptual
form:
