...
random variable that we filter out of the observed series.

The algorithm that accomplishes this filtering — the Hamilton filter — shares
the same prediction-correction logic as the Kalman filter introduced in
Chapter 11. Both process observations sequentially, form a prediction of the
current hidden state, update that prediction upon observing the data, and
produce a smoothed retrospective estimate of the state path. The difference
is that the Hamilton filter operates on a discrete state space (two or three
regimes) while the Kalman filter operates on a continuous one. Understanding
the Hamilton filter first makes the Kalman filter's structure immediately
recognisable.
