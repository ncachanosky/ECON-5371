---
title: "Chapter 10: Regime-Switching Models"
author: ""
abstract: |
...
unrate_raw = web.DataReader("UNRATE", "fred", start, end)
...
The National Bureau of Economic Research designates business cycle turning
...
rate — the annualised log difference of GDPC1 — looks qualitatively different
...
These costs are real but manageable. Hamilton's (1989) two-state model has
...
mean-switching with a common variance, following Hamilton (1989) exactly. We
...
prediction. Both steps are applications of Bayes' rule.
...
regime probabilities via Bayes' rule. The conditional density of $y_t$ given
...
(from the AR terms in equation (10.9)). Applying Bayes' rule:
...
back through the transition structure. The Kim (1994) smoother is the standard
implementation:
...
*can* observe. These are **threshold models**, and the key distinction is
...
**self-exciting TAR** or **SETAR**, introduced by Tong (1978) and developed
...
autoregression** (STAR), introduced by Chan and Tong (1986) and developed by
...
step applies Bayes' rule using a Gaussian likelihood evaluated at the current
...
multivariate analogue of the weight that Bayes' rule assigns to the new data
