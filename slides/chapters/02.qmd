...
> Is the economy doing well right now, or poorly?

- Not "what is the level of GDP" — we can always read that off directly
- The real question: is output *above* or *below* where it ought to be?
- "Where it ought to be" is not observed. It has to be **constructed**
- Every time series tells two stories simultaneously: where it's headed long-run, and where it stands right now relative to that path

::: {.eo-recap}
Separating those two stories — trend versus deviation from trend — is exactly what this chapter's tools do. The Fed's "output gap," the BLS's seasonally adjusted unemployment rate, every headline GDP number you read: all of them are decompositions, whether the reader realizes it or not.
:::
...
Two families of tools, one underlying goal — separate the predictable, structural part of a series from what's left over:
...
::: {.eo-recap}
This is not an academic exercise. Every major economic data release you have ever read in the news has already been passed through some form of decomposition before it reached you.
:::
...
## Running Example: NSA and SA Real GDP
...
::: {.eo-hazard}
**Units.** `GDPC1` is a Seasonally Adjusted Annual Rate (SAAR) — each quarter's value is multiplied by 4. `ND000334Q` is a raw quarterly *total*. Plotting them together without converting NSA to SAAR first makes the SA series look four times larger for no economic reason — a units error, not a seasonal one.
:::
...
No free smoothing parameter here — unlike the filters in Section 4, the averaging window is fixed by the seasonal period $m$ itself.
...
*Top: observed (charcoal) vs. 2×4 MA trend (copper) — closely tracking. Middle: seasonal factors — large ($\pm2$–3%) and stable across nearly eighty years. Bottom: the irregular remainder, where the business cycle signal lives — visible dips at every major postwar recession.*

::: {.notes}
Four things worth naming if time allows: smooth near-linear trend in logs
(consistent with balanced growth), large stable seasonal factors, and
recession dips in '74-75, '81-82, '08-09, '20. Don't over-narrate — let
the figure do the work, ~2 min max.
:::
...
> A policymaker watching raw retail sales in January sees a sharp drop from December — every single year, regardless of the economy's actual condition. How do you keep from reacting to noise?

Seasonal patterns are large, predictable, and economically uninteresting — which is exactly what makes them dangerous to look at directly.
...
The US Census Bureau's **X-13-ARIMA-SEATS** is the government standard. We won't run it — but understanding what it does is essential for interpreting official statistics correctly.
...
## X-13: The ARIMA and SEATS Components
...
::: {.eo-definition}
**ARIMA component.** Pre-extends the series by fitting an ARIMA model (Chapter 3) and forecasting several periods ahead, applies the moving-average filter to the *extended* series, then discards the forecasted values — eliminating the end-point problem.

**SEATS component.** An alternative: fit a SARIMA model, then decompose the series' spectrum directly into trend, seasonal, and irregular. Firmer statistical foundation; often smoother seasonal factors than X-11.
:::

X-13 is an industrial-grade production system — calendar effects, holiday effects, outlier detection, decades of accumulated refinement — not a simple function call.
...
::: {.eo-recap}
This is the first genuinely *forward-looking* tool in the chapter — everything before this point described the data; from here on, we're building forecasting machinery on top of that description.
:::
...
SES is appropriate for series with **no trend, no seasonality** — for a persistently trending series like GDP, it always lags, perpetually catching up.
...
\hat{b}_t       &= \beta(\hat{\ell}_t-\hat{\ell}_{t-1}) + (1-\beta)\hat{b}_{t-1} \\[4 pt]
\hat{y}_{t+h|t} &= \hat{\ell}_t + h\hat{b}_t
\end{align}
$$
...
**Forecast** = current level $+$ trend $\times$ horizon
:::
:::
...
- **Trend**: none (N), additive (A), additive damped (Ad), multiplicative (M or Md)
...
\hat{s}_t       &= \gamma(y_t-\hat{\ell}_{t-1}-\hat{b}_{t-1}) + (1-\gamma)\hat{s}_{t-m}   \\[4pt]
\hat{y}_{t+h|t} &= \hat{\ell}_t + h\hat{b}_t + \hat{s}_{t+h-m(k+1)}
\end{align}
$$
...
## The Full Taxonomy — And Which Cases Matter

Not all 30 (E,T,S) combinations are numerically stable or practically useful. Four matter most in macroeconomic work:

| Model | What it is |
|:---|:---|
| ETS(A,N,N) | Simple exponential smoothing |
| ETS(A,A,N) | Holt's linear method |
| ETS(A,Ad,N) | Holt's damped method — often the best all-around performer |
| ETS(A,A,A) / ETS(A,Ad,A) | Holt-Winters, undamped / damped |

::: {.eo-hazard}
The full taxonomy has **30 valid combinations** — not all numerically stable, most rarely used in practice. `statsmodels`' `ETSModel` and information criteria (AIC) handle the search; memorizing the full table isn't the point. Know the four above cold; recognize that the framework generalizes.
:::
...
::: {.eo-hazard}
**Prediction intervals are not confidence intervals.** A 95% PI says 95% of *future realizations* will fall in the band — it accounts for forecast uncertainty (future innovations), not parameter or model uncertainty. Estimated-parameter intervals in practice run slightly too narrow. Chapter 5 returns to calibrating this properly.
:::
...
## A First Honest Evaluation: Train-Test
...
::: {.eo-recap}
Large errors in 2020 Q1–Q2 reflect the pandemic being outside the support of any postwar-trained model — not a forecasting failure. The full machinery for honest evaluation is Chapter 5's job.
:::
:::
::: {}
...
::: {.eo-keyterms}
Decomposition
: $y_t=T_t+S_t+I_t$ (additive) or $T_t\times S_t\times I_t$ (multiplicative, equivalent to additive in logs).

Trend / Seasonal / Irregular
: The slow-moving level; systematic calendar-driven fluctuation with period $m$; the residual where the business cycle signal lives.

Classical decomposition
: Trend via centered $2\times m$ moving average, followed by averaging de-trended values by season.

Business cycle
: The deviation of output from its trend path — not a natural object, but a residual whose shape depends on the decomposition chosen.

HP filter / Hamilton filter
: A penalized-smoothness trend extractor governed by $\lambda$, vs. a regression-based cycle filter (project $y_{t+h}$ on four lags) with no end-point bias.

X-13-ARIMA-SEATS / STL
: The Census Bureau's institutional seasonal-adjustment standard (X-11 + ARIMA extension + SEATS), vs. the portable, transparent Loess-based alternative.

Exponential smoothing / Smoothing parameter $\alpha$
: Forecasts as geometrically-declining-weight averages of the past; $\alpha\in(0,1)$ governs how aggressively the level updates.

SES / Holt's method / Damped trend
: No-trend, no-seasonality smoothing (ETS(A,N,N)); adds a trend component (ETS(A,A,N)); multiplies the trend by $\phi\in(0,1)$ so forecasts converge (ETS(A,Ad,N)).

Holt-Winters / ETS framework
: Adds a seasonal smoothing equation (ETS(A,A,A) / ETS(A,Ad,A)); the unified (Error, Trend, Seasonality) taxonomy with likelihood-based estimation.

Prediction interval
: A horizon-widening range for $y_{T+h}$ reflecting forecast uncertainty — not parameter uncertainty, and not a confidence interval.
:::
